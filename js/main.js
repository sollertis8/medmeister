...
$(window).resize(function () {
    ...
    if (width <= 600) {
        $('#submit').removeClass('btn btn-info btn-lg').addClass('btn btn-info btn-sm');
        ...
        <div class="back">
            ...
            <div class="content">
                ...
                <div class="social-links text-center">
                    ...
                    doctor_distance="Dr. ${result.profile.last_name} is ${Math.round(result.practices[0].distance)} miles from you"
                    >View More</a>
                </div>
            </div>
        </div> <!-- end back panel -->
...
function displayResponseData(response) {
    if (response.data.length == 0) {
        $('.js-search-results').html(`<div class="no-results">Sorry, there were no results.  Try expanding your search area.</div>`);
...
function getPracticesFromDoctorData(data) {
    ...
    function getPracticeVisitAddress(practice) {
        ...
        return {
            street: (visitAddressKeys.indexOf('street') ? practice.visit_address.street : ""),
            ...
            state: (visitAddressKeys.indexOf('state') ? practice.visit_address.state : ""),
    ...
    for (var i = 0; i < unique_insurance_accepted.length; i++) {
        switch (unique_insurance_accepted[i]) {
            ...
            case 'First Choice Health':
                logo = "<img src='logos/first-choice-health.jpg' style='width: 50%; height: 50%;' />";
                break;
            case 'Providence Health  Services':
                logo = "<img src='logos/providence_health_services.jpeg' style='width: 50%; height: 50%;' />";
                break;
            case 'Delta Dental':
                logo = "<img src='logos/delta_dental.jpg' style='width: 25%; height: 25%;' />";
                break;
            case 'Cambia Health Solutions':
                logo = "<img src='logos/cambia_health_solutions.png' style='width: 25%; height: 25%;' />";
                break;
            case 'PacificSource Health Plans':
                logo = "<img src='logos/pacificsource_health.jpg' style='width: 25%; height: 25%;' />";
                break;
            case 'ODS Health Plan':
                logo = "<img src='logos/ODS_health.jpg' style='width: 25%; height: 25%;' />";
                break;
...
function getSpecialtiesFromDoctorData(data) {
    ...
    data.specialties.map(item => {
        ...
        specialties.push({
            ...
            description: (speacialtyKeys.indexOf('description') ? item.description : "")
        })
    })
    return specialties
}
...
function getLicensesFromDoctorData(data) {
    ...
    if (data.licenses.length != 0) {
        data.licenses.map(item => {
            ...
            licenses.push({
                state: (licenseKeys.indexOf('state') ? item.state : "No License Info Available")
            })
        })
    } else {
        licenses.push({
